...
def process_image(net, input_image):
    ...
    for idx, proposal in enumerate(data):
        if proposal[4] > 0:
            ...
            xmin = np.int(iw * (proposal[0] / 416))
            ymin = np.int(ih * (proposal[1] / 416))
            xmax = np.int(iw * (proposal[2] / 416))
            ymax = np.int(ih * (proposal[3] / 416))
            idx = int(proposal[5])
            detect_objs.append({
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('./video/beijing.mp4')
    ...
    while True:
        ...
        if not ret:
            break
        start = time.time()
        result = process_image(predictor, frame)
        end = time.time() - start
        # log.info(result)
        cv2.putText(frame, "Latency: {:.3f}".format(end * 1000) + "ms", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 0, 0), 2)
        cv2.putText(frame, "fps: {:.3f}".format(1 / end), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        if result != 0:
            r = ast.literal_eval(result)['objects']
            for i in range(0, len(r)):
                cv2.rectangle(frame, (r[i]['xmin'], r[i]['ymin']), (r[i]['xmax'], r[i]['ymax']),
                              (122 * i, 122 * (1 - i), 122 * (2 - i)), 2)
                name = r[i]['name']
                confidence = r[i]['confidence']
                cv2.putText(frame, '{}: {:.3f}'.format(name, confidence), (r[i]['xmin'], r[i]['ymin'] - 10),
    ...
    #     log.info(result)
    #
    #     # cv2.rectangle(frame, (1086, 384), (1248, 521), thickness=1)
